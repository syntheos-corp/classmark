...
try:
    from src.core.yolo_ocr import extract_text_with_yolo_ocr, YOLOTextDetector
    HAS_YOLO_OCR = True
except ImportError:
    HAS_YOLO_OCR = False

# OCR - Simple fallback (without YOLO)
try:
    from src.core.simple_ocr import extract_text_from_pdf_simple_ocr, should_use_ocr
    HAS_SIMPLE_OCR = True
except ImportError:
    HAS_SIMPLE_OCR = False

# Fast Pattern Matching - Aho-Corasick + Regex hybrid
try:
    from src.core.fast_pattern_matcher import FastPatternMatcher, HAS_AHOCORASICK
    HAS_FAST_MATCHER = True
except ImportError:
    HAS_FAST_MATCHER = False
    HAS_AHOCORASICK = False

# Visual Pattern Detection - LayoutLMv3
try:
    from src.core.visual_pattern_detector import VisualPatternDetector, HAS_LAYOUTLM
...
class TextExtractor:
    ...
    def extract_from_pdf_pdfplumber(self, file_path: str, max_pages: Optional[int] = None) -> Dict[str, str]:
        """
        ...
        try:
            ...
            with pdfplumber.open(file_path) as pdf:
                ...
                # Store total page count for confidence calculation
                ...
                if max_pages and max_pages < len(pdf.pages):
    ...
    def extract_from_pdf_pymupdf(self, file_path: str, max_pages: Optional[int] = None, start_page: int = 0) -> Dict[str, str]:
        ...
        try:
            ...
            with fitz.open(file_path) as doc:
                # Extract metadata
                if doc.metadata:
                    result['metadata'] = doc.metadata  # Already stringified in PyMuPDF

                # Store total page count for confidence calculation
                result['total_pages'] = len(doc)  # or doc.page_count

                # SECURITY: Sanity check for page count to prevent resource exhaustion
                if len(doc) > 10000:
            ...
            print(f"PyMuPDF: Corrupted PDF file {file_path}: {e}", file=sys.stderr)
        except fitz.FileNotFoundError as e:
            # File doesn't exist or not accessible
            print(f"PyMuPDF: File not found {file_path}: {e}", file=sys.stderr)
        except MemoryError as e:
            # PDF too large or memory exhaustion
            print(f"PyMuPDF: Memory error processing {file_path}: {e}", file=sys.stderr)
        except Exception as e:
            # Catch-all for other errors
            print(f"PyMuPDF: Unexpected error on {file_path}: {e}", file=sys.stderr)
    ...
    def has_text_layer(self, file_path: str) -> bool:
        """
        Fast check: Does PDF have extractable text layer?

        Only checks first page (milliseconds) to quickly determine if PDF
        is text-based or scanned. Avoids expensive full extraction when OCR
        is definitely needed.

        Returns:
            True if PDF has extractable text, False if scanned (needs OCR)
        """
        try:
            if HAS_PYMUPDF:
                with fitz.open(file_path) as doc:
                    ...
                    # If first page has 10+ words, likely has text layer
                ...
                # No PDF library available, assume needs OCR
                return False
        except Exception as e:
            print(f"Fast text layer check error on {file_path}: {e}", file=sys.stderr)
            return False  # Assume needs OCR on error
    ...
    def pdf_needs_ocr(self, extracted_data: Dict) -> bool:
        ...
        Note: This is called AFTER extraction. Use has_text_layer() for
        fast pre-extraction check.
        """
    ...
    def extract_from_pdf_with_ocr(self, file_path: str) -> Dict[str, str]:
        """
        Extract text from scanned PDF using SOTA OCR pipeline

        Priority order:
        1. YOLO + Tesseract (SOTA: YOLOv8n text detection + selective OCR)
        2. Simple OCR fallback (full-page Tesseract)
        3. Basic OCR (legacy method)
        """
        ...
        try:
            ...
            # Priority 1: Try YOLO + Tesseract OCR (SOTA)
            if HAS_YOLO_OCR:
                try:
                    print(f"Using YOLO + Tesseract OCR on {file_path}...", file=sys.stderr)
                    ocr_text, ocr_used = extract_text_with_yolo_ocr(
                        file_path,
                        fallback_text="",  # No fallback text, force OCR
                        word_threshold=0,   # Always use OCR when this method is called
                        use_gpu=False       # Use CPU for compatibility
                    )

                    if ocr_text.strip():
                        # Parse page markers to maintain structure
                        ...
                        for line in lines:
                            if line.startswith('[Page ') and line.endswith(']'):
                                ...
                                if current_page_num and current_page_text:
                                    result['body'].append(
                                        f"[PAGE {current_page_num} - YOLO OCR]\n" +
                                ...
                                # Extract page number
                                try:
                                    current_page_num = int(line.split()[1].rstrip(']'))
                                except:
                                    current_page_num = None
                            else:
                    ...
                    print(f"YOLO OCR error on {file_path}: {e}", file=sys.stderr)
                    print("Falling back to simple OCR...", file=sys.stderr)

            # Priority 2: Try simple OCR fallback
            if HAS_SIMPLE_OCR:
                try:
                    print(f"Using simple OCR on {file_path}...", file=sys.stderr)
                    ocr_text, success, proc_time = extract_text_from_pdf_simple_ocr(
                        file_path,
                        dpi=300,
                        max_pages=10
                    )
                    ...
                    print(f"Simple OCR error on {file_path}: {e}", file=sys.stderr)
                    print("Falling back to basic OCR...", file=sys.stderr)

            # Priority 3: Basic OCR (legacy method)
            print(f"Using basic OCR on {file_path}...", file=sys.stderr)
            ...
            print(f"PDF OCR error on {file_path}: {e}", file=sys.stderr)
    ...
    def extract(self, file_path: str, max_pages: Optional[int] = None, start_page: int = 0) -> Tuple[Dict[str, any], bool]:
        ...
        if ext == '.pdf':
            # Fast pre-check: Does PDF have text layer? (milliseconds)
            # Avoids expensive extraction when we know OCR is needed
            has_text = self.has_text_layer(file_path)

            if not has_text and HAS_OCR:
                # Scanned PDF detected - skip straight to OCR
                extracted = self.extract_from_pdf_with_ocr(file_path)
                ocr_used = True
            else:
...
class ClassificationScanner:
    ...
    def __init__(self, config: Dict):
        ...
        if HAS_FAST_MATCHER:
            ...
            print("⚠ Fast pattern matcher not available, using traditional regex", file=sys.stderr)

        # Initialize visual pattern detector (LayoutLMv3)
        if HAS_VISUAL_DETECTOR and config.get('use_visual_detection', True):
    ...
    def scan_text(self, text: str, extracted_data: Dict) -> List[Match]:
        ...
        if self.fast_matcher:
            # Use fast pattern matcher (Aho-Corasick + Regex hybrid)
            pattern_matches = self.fast_matcher.match_all(text)

            for pm in pattern_matches:
                match_text = pm.matched_text.strip()
                position = pm.start
            ...
            # Fallback to traditional regex pattern matching
        ...
        # Layer 7: Visual pattern detection (LayoutLMv3)
        # PROGRESSIVE ENHANCEMENT: Only use expensive visual detection if pattern matching
        # didn't find high-confidence matches
        if self.visual_detector and 'file_path' in extracted_data:
            file_path = extracted_data['file_path']

            # Only run on PDFs
            if file_path.lower().endswith('.pdf'):
                # Check if we already have high-confidence matches from pattern matching
                max_pattern_confidence = max([m.confidence for m in matches]) if matches else 0.0
                needs_visual_detection = max_pattern_confidence < 0.80  # Threshold for skipping visual

                try:
                    if needs_visual_detection:
                        print(f"  Running visual detection (pattern confidence: {max_pattern_confidence:.2f})", file=sys.stderr)

                        # Initialize detector lazily (only when needed)
                        if not self.visual_detector._initialized:
                            self.visual_detector.initialize()

                        # Run visual pattern detection
                        visual_matches = self.visual_detector.detect_visual_patterns(file_path)
                    else:
                        print(f"  Skipping visual detection (pattern confidence: {max_pattern_confidence:.2f} >= 0.85)", file=sys.stderr)
                        visual_matches = []

                    if visual_matches:
                        print(f"  Visual detector found {len(visual_matches)} pattern matches", file=sys.stderr)
                        ...
                        for visual_match in visual_matches:
                            ...
                            if not is_covered:
                                ...
                                # Boost for banner/header patterns
                                if visual_match.region.is_banner:
                                    confidence += 0.15
                                elif visual_match.region.is_header:
                                    confidence += 0.10

                                # Boost for authority blocks
                                if visual_match.pattern_type == 'authority_block':
                                    confidence += 0.15
                                ...
                                threshold = self.config.get('confidence_threshold', 0.3)
                                if confidence >= threshold:
                                    # Determine location from visual region
                                    if visual_match.region.is_header:
                                        location = 'header'
                                    elif visual_match.region.is_footer:
                                        location = 'footer'
                                    else:
                                        location = 'body'
                                    ...
                                    matches.append(Match(
                                        ...
                                        position=-1,  # Visual matches don't have text position
                                        line_number=visual_match.region.page_num,  # Use page number as line number
                                        confidence=confidence,
                                        context=f"[Page {visual_match.region.page_num}] {visual_match.pattern_type}",
                                        match_type=f"visual_{visual_match.pattern_type}",
                                        location=location
                                    ))
    ...
    def scan_file(self, file_path: str, enable_early_exit: bool = True,
                  early_exit_threshold: float = 0.90,
                  quick_scan_pages: int = 3) -> DetectionResult:
        """
        Scan a single file for classification markings with optional early exit

        Args:
            file_path: Path to file to scan
            enable_early_exit: If True, attempt early exit on high-confidence matches (default: True)
            early_exit_threshold: Confidence threshold for early exit (default: 0.90)
            quick_scan_pages: Number of pages to check in quick scan (default: 3)

        Returns:
            DetectionResult with classification information

        Performance Notes:
            - Early exit can provide 10-100x speedup for large classified documents
            - Documents with clear first-page banners will exit after 1-3 pages
            - Clean documents will still be fully scanned for accuracy
        """
        import time
        ...
        # Determine if we should attempt early exit (only for PDFs)
        is_pdf = file_path.lower().endswith('.pdf')
        attempt_early_exit = enable_early_exit and is_pdf

        # OPTIMIZATION 1: Quick scan of first N pages for PDFs
        if attempt_early_exit:
            # Extract only first N pages
            quick_data, quick_ocr = self.extractor.extract(file_path, max_pages=quick_scan_pages)

            # Quick check on first pages
            quick_text_parts = []
            if 'headers' in quick_data:
                quick_text_parts.extend(quick_data['headers'])
            if 'body' in quick_data:
                quick_text_parts.extend(quick_data['body'])
            if 'footers' in quick_data:
                quick_text_parts.extend(quick_data['footers'])

            quick_text = '\n\n'.join(str(part) for part in quick_text_parts if part)

            # Scan quick text for high-confidence matches
            if quick_text.strip():
                quick_data['file_path'] = file_path
                quick_matches = self.scan_text(quick_text, quick_data)

                # Check if we found high-confidence classification
                if quick_matches:
                    max_confidence = max([m.confidence for m in quick_matches])

                    # EARLY EXIT: High confidence classification found
                    if max_confidence >= early_exit_threshold:
                        classification_level = self.determine_classification_level(quick_matches)

                        # Only exit early for actual classification levels (not UNKNOWN)
                        if classification_level not in [ClassificationLevel.UNCLASSIFIED,
                                                       ClassificationLevel.UNKNOWN]:
                            processing_time = time.time() - start_time
                            total_pages = quick_data.get('total_pages', quick_scan_pages)

                            print(f"Early exit at page {quick_scan_pages}/{total_pages} "
                                  f"(confidence: {max_confidence:.2f}, level: {classification_level.value})",
                                  file=sys.stderr)
        ...
        # Add file_path to extracted_data for visual pattern detector
